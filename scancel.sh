...
tSDRG_re="${tSDRG_path}/tSDRG/Main_${Spin}/rm_submit/"

echo -e "$tSDRG_re"


if [ -d "${tSDRG_re}""${now_date}" ]; then
    # 目錄 /path/to/dir 存在
    echo -e "${tSDRG_re}""${now_date}"
else
    # 目錄 /path/to/dir 不存在
    echo -e "mkdir""${tSDRG_re}""${now_date}"
    mkdir -p "${tSDRG_re}""${now_date}"
fi
...
fileDir=${tSDRG_re}${now_date}${fileName}
        ...
        do
                ...
                do
                        ...
                        do      
                                ...
                                name="spin"${Spin}"_L"${L}"_"${Jdis}"_"${Dim}"_B"${bonDim}"_BC="${BC}"_Ncore="${Ncore}"_seed1="${localS1}"_seed2="${localS2}"_random"
                                echo -e ${name}
