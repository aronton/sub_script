...
def get_dx_frame(dx, spin, originalframe, L, J, D, seed):

    # dx seperate directory
    bulk_dx_seperate_dir_path = "/home/aronton/tSDRG_random/Sorting_data/Spin" + str(spin) + "/metadata/bulk_dx_seperate/" + str(J) + "/" + str(D) + "/" + "L" + str(L) + "/dx_" + str(dx) + "/"
    ...
    print("seperate:%d,%s,%s,%s,%d" %(dx,L,J,D,seed))
...
def get_ave_frame(dx, spin, L, J, D, intial_seed, final_seed):
    ...
    bulk_dx_accumulate_dir_path = "/home/aronton/tSDRG_random/Sorting_data/Spin" + str(spin) + "/metadata/bulk_dx_accumulate/" + str(J) + "/" + str(D) + "/" + "L" + str(L) + "/dx_" + str(dx) + "/"

    if(os.path.exists(bulk_dx_accumulate_dir_path) == False):
        os.makedirs(bulk_dx_accumulate_dir_path)
    ...
    # dx accumulate csv file path
    bulk_dx_accumulate_csv_path = bulk_dx_accumulate_dir_path + "/L" + str(L) + "_" + str(J) + "_" + str(D) + "_dx_" + str(dx) + ".csv"

    # bulk_dx_accumulate_path = bulk_dx_accumulate_path + "/L" + str(L) + "_" + str(J) + "_" + str(D) + "_dx_" + str(dx) + "_seed_" + str(final_seed) + ".csv"


    # print("bulk_dx_accumulate_csv_path\n",bulk_dx_accumulate_csv_path)
    ...
    for seed in range(intial_seed, final_seed + 1):
        ...
        # bulk_dx_accumulate_frame['seed'][int(seed)-1] = int(seed)
    print("bulk_dx_accumulate_frame", bulk_dx_accumulate_frame)
    # bulk_dx_accumulate_frame['seed'] = bulk_dx_accumulate_frame['seed'].astype('int')
    bulk_dx_accumulate_frame.to_csv(bulk_dx_accumulate_csv_path, index = False)
    ...
    ave_dir = "/home/aronton/tSDRG_random/Sorting_data/Spin" + str(spin) + "/metadata/bulk_ave/" + str(J) + "/" + str(D) + "/" + "L" + str(L) + "/dx_" + str(dx) + "/"

    if(os.path.exists(ave_dir) == False):
        os.makedirs(ave_dir) 

    # average csv path
    ave_csv = "/home/aronton/tSDRG_random/Sorting_data/Spin" + str(spin) + "/metadata/bulk_ave/" + str(J) + "/" + str(D) + "/" + "L" + str(L) + "/dx_" + str(dx) + "/" + "bulk_ave_L" + str(L) + "_" + str(J) + "_" + str(D) + "_dx_" + str(dx) + ".csv"
...
# # dx csv file
# bulk_dx_seperate_csv_path = bulk_dx_seperate_dir_path + "L" + str(L) + "_" + str(J) + "_" + str(D) + "_dx_" + str(dx) + "_seed_" + "temp" +".csv"
...
# # dx accumulate csv file path
# bulk_dx_accumulate_path = bulk_dx_accumulate_path + "/L" + str(L) + "_" + str(J) + "_" + str(D) + "_dx_" + str(dx) + "_seed_" + "temp" + ".csv"
# for dx in dx_array:
...
for seed_num in range(initial_Seed, final_Seed + 1):
    ...
    # path of source file
    source_csv = '/home/aronton/tSDRG_random/tSDRG/Main_' + str(spin) + '/data_random/'+ BC +'/'+ jdis + '/'+ dim + '/L'+ str(L) +'_P'+ str(probDis) +'_m'+ str(chi) +'_'+ str(seed_num) + '/L'+ str(L) +'_P'+ str(probDis) +'_m'+ str(chi) +'_'+ str(seed_num) + "_corr1" + '.csv'
    ...
    with mp.Pool(cpus) as p1:
        p_x1 = partial(get_dx_frame, spin=spin, originalframe=corr_frame, L=L, J=jdis, D=dim, seed=seed_num)
...
with mp.Pool(cpus) as p2:
    p_x2 = partial(get_ave_frame, spin=spin, L=L, J=jdis, D=dim, intial_seed=initial_Seed, final_seed=final_Seed)
